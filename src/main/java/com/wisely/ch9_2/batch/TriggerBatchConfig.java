...
public class TriggerBatchConfig {
	...
	public ItemProcessor<Person, Person> processor() {
		CsvItemProcessor processor = new CsvItemProcessor(); //1、使用我们自己定义的ItemProcessor的实现CsvItemProcessor。
		processor.setValidator(csvBeanValidator()); //2、为processor指定校验器为CsvBeanValidator
		return processor;
	}
	...
	@Bean
	public ItemWriter<Person> writer(DataSource dataSource) {//1、Spring能让容器中已有的Bean以参数的形式注入，SpringBoot已为我们定义了dataSource。
		JdbcBatchItemWriter<Person> writer = new JdbcBatchItemWriter<Person>(); //2、我们使用JDBC批处理的JdbcBatchItemWriter来写数据到数据库
		writer.setItemSqlParameterSourceProvider(new BeanPropertyItemSqlParameterSourceProvider<Person>());
		String sql = "insert into person " + "(id,name,age,nation,address) "
				+ "values(hibernate_sequence.nextval, :name, :age, :nation,:address)";
		writer.setSql(sql); //3、在此设置要执行批处理的SQL语句
		writer.setDataSource(dataSource);
		return writer;
	}

	/**
	 * jobRepository的定义需要dataSource和transactioManager，SpringBoot已为我们自动配置了这两个类，Spring可通过方法注入已有的Bean。
	 * @param dataSource
